...
if [ $account_id != 664749818226 ]; then
    echo ''
    echo '###############################################################################################################################'
    echo 'Account mismatch please use:'
    echo 'aws-vault exec govcloud-production-<engineers|poweruser> -- chamber exec datadog -- terragrunt <init|plan|apply|output|destroy>'
    echo '###############################################################################################################################'
    echo ''
